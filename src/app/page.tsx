...
import { 
  ...
  Layers, Bell, X, AlertTriangle, Loader2, CheckCircle2, Image as ImageIcon, Inbox, Settings as SettingsIcon, HelpCircle 
} from "lucide-react";
...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    if (theme && theme.css) {
        ...
        // SPECIAL CASE FOR CYBER HIGH CONTRAST
        if (theme.name === "Cyber (HC)") {
             // FORCING BRAND-TEXT
        }
    } else {
  ...
  const items: Item[] = useMemo(() => {
    ...
    return list;
  }, [itemsData, filter.kind, recentLimit]);
  ...
  return (
    <div className="flex h-screen w-screen bg-surface-50 dark:bg-surface-950 text-surface-900 dark:text-surface-50 font-sans antialiased overflow-hidden">
      <CmdK
        onPick={(id) => {
          setSelectedId(id);
          setFilter({ kind: "all" });
      ...
      {!zenMode && (
          <aside 
            className="flex-shrink-0 border-r border-surface-200 dark:border-surface-800 relative flex flex-col animate-fade-in-left transition-all duration-300"
            style={{ width: sidebarCollapsed ? 0 : sidebarWidth, opacity: sidebarCollapsed ? 0 : 1, overflow: sidebarCollapsed ? 'hidden' : 'visible' }}
          >
            {/* BACKDROP OPACITY */}
            ...
            <div className="p-4 h-full flex flex-col relative z-10">
                ...
                >
                    ...
                    >
                        ...
                        <span className="text-[13px] font-bold tracking-[0.15em] uppercase" style={{ color: theme === 'custom' ? 'var(--title-color)' : undefined }}>LocalSnips</span>
                    </div>
                ...
                <div className="mt-auto px-2 pb-2 pt-2 border-t border-surface-200 dark:border-white/5 flex flex-col gap-1">
                         <button
                            onClick={() => { setFilter({ kind: "settings" }); }}
                            className={clsx(
      ...
      {!zenMode && sidebarCollapsed && (
        <button
          ...
          className="fixed z-40 left-0 top-3/4 -translate-y-1/2 w-5 h-24 flex items-center justify-center bg-surface-100/90 dark:bg-surface-800/90 hover:bg-surface-200 dark:hover:bg-surface-700 border-r border-t border-b border-surface-200 dark:border-surface-700 rounded-r-xl transition-all backdrop-blur-sm shadow-sm group"
          data-tooltip="Show Sidebar"
        >
          <ChevronRight className="w-3.5 h-3.5 text-surface-400 group-hover:text-surface-600 dark:group-hover:text-surface-300 transition-colors" />
        </button>
      )}
      ...
      {!zenMode && !sidebarCollapsed && (
        <ResizableDivider
          onResize={(delta) => setSidebarWidth(Math.max(200, Math.min(400, sidebarWidth + delta)))}
        />
      )}
      ...
      <div 
        className={cn("flex-shrink-0 border-r border-surface-200 dark:border-surface-800 bg-surface-50 dark:bg-surface-950 flex flex-col min-w-0 transition-all duration-300", zenMode ? "w-0 opacity-0 overflow-hidden border-none" : "")}
        style={{ width: zenMode ? 0 : listWidth }}
      >
        <div className="p-4 border-b border-surface-100 dark:border-surface-800 bg-surface-50/80 dark:bg-surface-950/80 backdrop-blur-sm z-10 space-y-4">
          <ImportBox
            onCreated={(id) => {
              mutateItems();
              setSelectedId(id);
      ...
      {!zenMode && (
        <ResizableDivider
          onResize={(delta) => setListWidth(Math.max(300, Math.min(600, listWidth + delta)))}
        />
      )}

      <main className="flex-1 min-w-0 bg-surface-50 dark:bg-surface-950 flex flex-col overflow-hidden relative">
        <div className="absolute top-4 right-4 z-40 flex items-center gap-2">
             <button 
                onClick={() => setZenMode(!zenMode)}
                className="p-2 rounded-lg bg-surface-100/50 hover:bg-surface-200 text-surface-500 hover:text-surface-900 transition-all backdrop-blur-sm"
...
function AppearanceView({ 
  ...
  return (
    ...
    <AlertModal 
        ...
        isDestructive
    />
    <div className="max-w-4xl mx-auto w-full p-12 animate-fade-in h-full overflow-y-auto">
      <div className="flex flex-col items-center text-center mb-12 animate-fade-in-down">
        <div className="w-16 h-16 rounded-[1.2rem] bg-brand-50 dark:bg-surface-800 flex items-center justify-center text-brand-600 dark:text-surface-100 shadow-sm mb-5 border border-transparent dark:border-white/10">
            <Palette className="w-8 h-8" />
        </div>
        <div>
            <h1 className="text-3xl font-bold text-surface-500 dark:text-surface-400 tracking-tight mb-2">Appearance</h1>
            <p className="text-base text-surface-500 dark:text-surface-400 font-medium leading-relaxed max-w-md mx-auto text-balance">
                        ...
                        {[
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    ? "bg-white dark:bg-surface-600 text-surface-900 dark:text-white shadow-sm ring-1 ring-black/5 dark:ring-white/10"
                                    ...
                                    <div className="p-4 bg-surface-100/50 dark:bg-surface-800/30 rounded-xl border border-surface-200/50 dark:border-white/5 space-y-4">
                                        <div className="grid grid-cols-8 gap-2">
                                            {[
                                                ...
                                                <button
                                                    key={bg.hex}
                                                    onClick={() => themeHook.setCustomTheme({ ...themeHook.customTheme, darkBg: bg.hex })}
                                                    className="group relative w-full aspect-square rounded-full border-2 border-surface-200 dark:border-white/10 shadow-sm transition-transform hover:scale-105"
                                                    title={bg.name}
                                                    style={{ backgroundColor: bg.hex }}
                                                >
                                                    {themeHook.customTheme.darkBg === bg.hex && (
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="group relative">
                                                ...
                                                >
                                                    Color for section headers and category titles. Presets adjust based on your window tint.
                                                </div>
                                            </div>
                                        </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-surface-50 dark:bg-surface-950 rounded-xl border border-surface-100 dark:border-surface-800">
                        ...
                        <input 
                            ...
                            min="1" 
                            max="20"
                            className="w-20 px-3 py-2 rounded-lg border border-surface-300 dark:border-surface-700 bg-white dark:bg-surface-900 text-center font-mono text-sm focus:ring-2 focus:ring-brand-500 outline-none dark:text-surface-100"
                            value={recentLimit}
                            onChange={(e) => onLimitChange(parseInt(e.target.value) || 4)}
                        />
                        </div>
                </div>
            </div>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between p-4 bg-surface-50 dark:bg-surface-950 rounded-xl border border-surface-100 dark:border-surface-800">
                    ...
                    <button 
                        ...
                        className="px-4 py-2 bg-surface-100 dark:bg-surface-800 hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-700 dark:text-surface-200 rounded-lg text-sm font-medium transition-colors border border-surface-200 dark:border-surface-700"
                    >
                        Reset
                    </button>
                </div>
